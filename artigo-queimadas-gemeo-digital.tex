...
Implementamos este operador como uma rede neural codificadora-decodificadora (KoopmanAE). O codificador $g_\phi: \mathbb{R}^n \to \mathbb{R}^d$ projeta o estado observado em observáveis latentes $\mathbf{z}_t = g_\phi(\mathbf{x}_t)$. A evolução linear no espaço latente é:
...
A inovação principal é a regularização da perda da GNN com a equação física de propagação de fogo de Rothermel~\cite{article:rothermel1972}. A velocidade de propagação $R$ (m/min) é modelada como:
...
% Figura INOV-006: Diagrama Koopman-PI-GNN
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/diagrama-koopman-pignn.png}
\caption{Arquitetura do modelo híbrido NeKo-PIGNN. Dados de satélite (VIIRS/GOES-16) passam por pré-processamento e codificação via autoencoder variacional para o espaço de observáveis de Koopman. A matriz K propaga a dinâmica temporal, enquanto a PI-GNN modela a propagação espacial com regularização física de Rothermel. A perda total combina reconstrução, predição e termo PDE.}
\label{fig:diagrama-koopman}
\end{figure}

% Tabela INOV-008: Benchmark
