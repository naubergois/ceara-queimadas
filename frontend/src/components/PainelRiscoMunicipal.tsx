  ...
  return (
    <section className="card space-y-3" aria-label="Ranking de risco municipal">
      <div className="flex items-center gap-2 mb-1">
        <TrendingUp size={16} className="text-orange-400" aria-hidden="true" />
        <h2 className="text-sm font-semibold text-white">Risco por Município</h2>
      </div>
            ...
            <li key={r.municipio} className="space-y-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-xs text-gray-500 w-5 shrink-0">{r.posicao}.</span>
                  <span className="text-sm text-gray-200 truncate">{r.municipio}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs text-gray-400">{r.focos_24h} focos</span>
                  <span className={classConfig[r.classificacao]}>{r.classificacao}</span>
                </div>
              </div>
              {/* Barra de risco */}
              <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden" role="progressbar" aria-valuenow={r.indice_risco} aria-valuemin={0} aria-valuemax={100}>
                <div
              ...
              {/* Justificativa */}
              {r.justificativa && (
                <p className="text-xs text-gray-500 leading-tight">{r.justificativa}</p>
              )}
            </li>
          ))}
        </ol>
