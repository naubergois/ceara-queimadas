...
def fuse_masks_majority_vote(masks: Sequence[np.ndarray], min_votes: int) -> np.ndarray:
    """
    Voto por célula: marca positivo se pelo menos ``min_votes`` máscaras coincidem.

    **Consenso multi-vista** não supervisionado: reduz alarmes quando apenas um
    caminho (nuvem, solo quente pontual, artefacto) dispara.
    """
    if not masks:
        raise ValueError("fuse_masks_majority_vote requer pelo menos uma máscara")
    mv = int(min_votes)
    if mv < 1:
        raise ValueError("min_votes deve ser >= 1")
    if mv > len(masks):
        raise ValueError("min_votes não pode exceder o número de máscaras")
    acc = np.zeros_like(masks[0], dtype=np.int16)
    for m in masks:
        acc += m.astype(np.int16)
    return acc >= mv
