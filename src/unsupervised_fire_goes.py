...
**Linha E — consenso multi-vista:** método ``consensus_ensemble`` (voto 2/3 entre gêmeo digital,
``combined_persistence`` e residual espacial multi-banda) para reduzir FP de um único detector.
"""
...
def detect_consensus_ensemble(
    ...
    *,
    ...
    """
    **Consenso multi-vista (Linha E)** — combina três hipóteses físicas independentes na mesma grade:

    1. **Gêmeo digital** (assimilação temporal ``prob_or`` / percentil no risco suavizado).
    2. **combined_persistence** (pico + média + persistência horária).
    3. **Residual espacial multi-banda** (T_B13 vs fundo local + BTD quando há 7/14).

    Célula marcada como fogo só se ``>= consensus_min_votes`` dos três concordam (por defeito **2 em 3**).
    Isto é **não supervisionado** na inferência; tende a **subir precisão** ao custo de recall quando um
    só detector é enganado por nuvem ou solo quente isolado.
    """
    bt13 = band_grids.get(13)
    if bt13 is None:
        raise ValueError("band_grids deve incluir canal 13")
    ...
    m_t = twin.predict_mask(float(contamination), valid_bins)
    m_c = detect_combined_persistence(
        hourly_grids,
        valid_bins,
        float(contamination),
        cfg=combined_persistence_cfg,
    )
    m_s = detect_spatial_residual_multiband(bt13, bt7, bt14, contamination=float(contamination))
    masks = [
        m_t & valid_bins,
        m_c & valid_bins,
        m_s & valid_bins,
    ]
    return fuse_masks_majority_vote(masks, int(consensus_min_votes))
...
def calibrate_contamination(
    ...
    *,
    ...
    twin_cfg: Optional[GOESFireDigitalTwinConfig] = None,
    combined_persistence_cfg: Optional[CombinedPersistenceConfig] = None,
    band_grids: Optional[Dict[int, np.ndarray]] = None,
    consensus_min_votes: int = 2,
    ...
    for c in np.linspace(0.004, 0.18, n_steps):
        if method == "spatial_residual":
            ...
            if hourly_slots and band_grids is not None:
                pred = detect_consensus_ensemble(
                    hourly_slots,
                    band_grids,
                    valid_bins,
                    float(c),
...
def find_local_goes_nc(
    ...
    *,
    ...
    """
    Localiza NetCDF por dia/canal. Se ``hour_utc`` for dado, filtra pelo prefixo temporal ABI
    ``_s{YYYY}{DDD}{HH}`` no nome (evita apanhar sempre o primeiro granulo do dia na ordenação).
    """
    ...
    candidates = sorted(p for p in raw_dir.glob("*.nc") if tag in p.name and tag_ch in p.name)
    if hour_utc is not None:
        needle = f"_s{day_utc.year}{doy:03d}{int(hour_utc):02d}"
        narrowed = [p for p in candidates if needle in p.name]
        if narrowed:
            candidates = narrowed
    if not candidates:
        raise FileNotFoundError(
            f"Nenhum NetCDF em {raw_dir} com '{tag}' e canal '{tag_ch}'"
            + (f" e hora≈{hour_utc} UTC" if hour_utc is not None else "")
            + " (--skip-download)."
        )
    return candidates[0]
...
def collect_hourly_band_grids(
    ...
    *,
    ...
    if skip_download and len(hours_utc) > 1:
        warnings.warn(
            "Com --skip-download são escolhidos NetCDF por hora UTC no nome "
            "(prefixo ``_s{ano}{día}{hora}``). Falta de ficheiro por hora falha com erro.",
            stacklevel=2,
        )
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    p.add_argument(
        ...
        default="all",
        help="both=spatial+IF; all=+twin+combined+consensus (Linha E=voto 2/3, doc METODOLOGIA_NOVA_PROPOSTA.md)",
    )
    p.add_argument(
        "--consensus-min-votes",
        type=int,
        default=2,
        help="Para consensus_ensemble: mínimo de detectores em acordo (1–3; defeito 2)",
    )
    ...
    results = run_dates_cli(
        ...
        combined_persistence_cfg=combined_persistence_cfg,
        calibrate_beta=calibrate_beta,
        consensus_min_votes=max(1, min(3, int(args.consensus_min_votes))),
    )
    for r in results:
