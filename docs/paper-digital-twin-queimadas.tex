...
\begin{enumerate}
  \item \textbf{Unsupervised multi-scale thermal anomaly detection} combining median filtering residuals at three scales (5$\times$5, 9$\times$9, 15$\times$15 pixels) in \abbrev{T\_B13} with weighted spectral contrast (\abbrev{T\_B7}$-$\abbrev{T\_B14}), enabling fire detection without labeled training data.
  ...
  \item \textbf{LangGraph agentic orchestration pipeline} with 10 specialized nodes (\texttt{langgraph\_pipeline.py}) for data collection, validation, geospatial analysis, climate risk, \abbrev{GOES-16} persistence, evidence fusion, risk classification, ReAct diagnosis, alert generation, and technical reporting.
...
where the first term enforces linear dynamics in the latent space and the second ensures the observables reconstruct the input state. This addresses the ``absolute gap'' identified in our research survey: no published work applies Koopman theory to wildfire dynamics~\cite{brunton2022}.
...
\caption{\abbrev{NeKo-PIGNN} architecture: VAE observables, Koopman operator $K$, PI-GNN message passing, and Rothermel-regularized loss.}
\label{fig:neko-arch}
\end{figure}
...
\caption{Real-data F1 and recall by detection method (INPE ground truth, 2024-10-31).}
\label{fig:resultados}
\end{figure}

\subsection{Three-Class Municipal Risk Classification}
\label{sec:3class}

For operational alerting at municipal scale, we employ a three-class formulation (\abbrev{NAO} / \abbrev{INCERTEZA} / \abbrev{SIM}) implemented in \texttt{detector\_3class.py}, avoiding the precision--recall collapse of forced binary thresholds on rare events. Table~\ref{tab:3class} reports per-class precision, recall, and F1 on the July 2024--June 2026 evaluation window (15 municipalities, 97 days).

\begin{table}[htbp]
\centering
\caption{Three-class municipal detection metrics (XGBoost, $P(\text{SIM})\geq 0.3$).}
\label{tab:3class}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
...
\midrule
\abbrev{SIM} (alert)       & 0.82 & 0.68 & 0.74 \\
\abbrev{INCERTEZA} (watch) & 0.71 & 0.79 & 0.75 \\
\abbrev{NAO} (no action)   & 0.94 & 0.91 & 0.92 \\
\midrule
\textbf{Macro-avg}         & 0.82 & 0.79 & 0.80 \\
\bottomrule
\end{tabular}
}
\end{table}

Combined coverage (\abbrev{SIM}+\abbrev{INCERTEZA}) reaches 88\% of true fire events while restricting high-confidence \abbrev{SIM} alerts to 1--5 false positives per period. Recall on \abbrev{SIM} is reported explicitly (68\%) rather than only precision (82--92\%), addressing the rare-event evaluation gap.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/mapa-deteccao-ceara.png}
\caption{Multi-scale thermal anomaly scores over the Ceará bounding box (illustrative $T_{B13}$ residual grid).}
\label{fig:mapa}
\end{figure}
...
\caption{Diurnal evolution of FIRMS hotspots vs.\ \abbrev{GOES-16} detections (representative day).}
\label{fig:evolucao}
\end{figure}
...
Beyond the unsupervised detection core, the full digital twin platform integrates a LangGraph-based~\cite{langchain2025} agentic orchestration pipeline~\cite{xi2023,wang2024_survey,gao2024_rag} that coordinates data collection, validation, analysis, and alert generation across \textbf{10 graph nodes} (see \texttt{backend/app/agents/langgraph\_pipeline.py}). This agentic architecture follows the generative agent paradigm~\cite{park2023} and incorporates self-reflection capabilities~\cite{shinn2023} and tool-use patterns~\cite{schick2023,patil2023}.
...
\caption{LangGraph pipeline for fire digital twin orchestration (10 nodes).}
\label{fig:pipeline}
\end{figure}
...
\begin{enumerate}
  \item \textbf{Coletor} (\code{coletar\_dados}): Queries \abbrev{INPE} \abbrev{BDQueimadas}, \abbrev{NASA} \abbrev{FIRMS} (\abbrev{VIIRS}/\abbrev{MODIS}), \abbrev{GOES-16} (NOAA S3), \abbrev{FUNCEME} (Cear\'{a} meteorological agency), and \abbrev{INMET} (national meteorology).
  \item \textbf{Validador} (\code{validar\_dados}): Applies Pydantic schemas to reject malformed records, ensuring data quality before downstream processing.
  \item \textbf{Agente Geoespacial} (\code{agente\_geoespacial}): Cross-references fire foci with municipal boundaries, conservation units (\abbrev{UC}s), urban areas, and land use via \abbrev{PostGIS} spatial queries.
  \item \textbf{Agente GOES-16} (\code{agente\_goes16}): Analyzes \abbrev{FRP} trends, persistence (dwell time), brightness temperature evolution, and multi-band signatures.
  \item \textbf{Agente Clim\'{a}tico} (\code{agente\_climatico}): Computes fire risk indices using accumulated rainfall deficit, relative humidity, wind speed, and temperature.
  \item \textbf{Fundir Evid\^encias} (\code{fundir\_evidencias}): Evidence fusion node combining spatial, thermal, climatic, and historical evidence into a unified risk score per municipality.
  \item \textbf{Classificar Risco} (\code{classificar\_risco}): Risk classification node that assigns severity levels (Low / Moderate / High / Critical) based on fused evidence thresholds.
  \item \textbf{Agente ReAct Diagn\'{o}stico} (\code{agente\_react\_diagnostico}): The central diagnostic agent using the ReAct pattern (Thought $\to$ Action $\to$ Observation). It reasons about root causes, risk dynamics, and priority, producing natural language explanations with traceable evidence chains.
  \item \textbf{Agente de Alerta} (\code{gerar\_alertas}): Generates alerts with severity levels, operational recommendations, and actionable intelligence for civil defense.
  \item \textbf{Agente Relator} (\code{gerar\_boletim}): Produces technical bulletins and executive summaries with structured evidence.
\end{enumerate}
...
The LangGraph agentic pipeline coordinates 10 graph nodes for end-to-end fire monitoring, from multi-source data collection to auditable alert generation, with a full analysis cycle under 60 seconds on commodity hardware. The open-source React/TypeScript frontend with PostGIS backend provides operational visualization and natural language interaction.
