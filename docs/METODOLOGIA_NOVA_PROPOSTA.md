...
## Linha E — **Consenso multi-vista** (inovação operacional)

**Problema:** Um único detector (só percentil espacial, só twin, só persistência) é vulnerável a **um modo de falha** — nuvem fina, solo urbano quente, granulo isolado.

**Ideia:** Três **visões físicas quase independentes** na mesma grade; marcar fogo só onde há **acordo por voto**:

| Vista | O que mede |
|-------|------------|
| **Gêmeo digital** | Estado assimilado ao longo das horas (`prob_or`, risco suavizado). |
| **combined_persistence** | Pico + média temporal + persistência de activações horárias. |
| **Residual espacial multi-banda** | Anomalia T_B13 vs fundo local + BTD (7−14) na grade agregada. |

Por defeito **2 em 3** votos (`fuse_masks_majority_vote`). Isto tende a **aumentar precisão** (menos FP espúrios) ao preço de algum recall se apenas uma vista “vê” o fogo real — compensação típica em operações onde alarmes falsos são caros.

Implementação: **`consensus_ensemble`** (`detect_consensus_ensemble` em `src/unsupervised_fire_goes.py`), `--consensus-min-votes 2`.

---

## Avaliação **imagem ↔ focos INPE** (pixéis ABI)

A grade **72×72** compara caixotes com pontos INPE e tende a **subestimar** deteção útil.
O script ``python -m src.pixel_inpe_eval`` trabalha na **resolução do recorte CE** no NetCDF:

- score multi-banda por hora + fusão ``max`` no tempo;
- limiar por ``contamination`` nos pixéis válidos;
- clusterização espacial e correspondência **foco ↔ centroide** dentro de **``match_radius_km``** (Haversine);
- métricas **precision / recall / F1** ao nível de **pares foco–cluster** (não células da grade).

Com ``--skip-download``, os ficheiros locais devem incluir o **granulo por hora** (filtro ``_s{YYYY}{DOY}{HH}`` no nome).
...
Comparar com `--method all` (inclui **consensus_ensemble**, Linha E) ou só consenso:
