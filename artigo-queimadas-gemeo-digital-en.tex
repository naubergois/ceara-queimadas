...
Wildfire monitoring based on satellite data is a consolidated area. INPE operates BDQueimadas \cite{article:inpe2023}, which collects data from AQUA, TERRA, NPP, and NOAA-20 satellites. NASA FIRMS \cite{manual:firms2024} provides VIIRS and MODIS data in public CSV format with no API key requirement. GOES-16, maintained by NOAA, provides near real-time data via public S3 bucket \cite{techreport:goes2022}.

The Digital Twin paradigm, originally conceived for manufacturing \cite{article:grieves2014,article:tao2019}, has been expanded to environmental and Earth systems \cite{article:bauer2021nature,article:bauer2023natrev,article:nativi2021}, culminating in initiatives such as the European Destination Earth \cite{article:voosen2020}. Comprehensive surveys have consolidated definitions, characteristics, and challenges of the technology \cite{article:barricelli2019,article:rasheed2020,article:fuller2020,article:jones2020}.
...
\begin{itemize}
    \item Convolutional neural networks for smoke detection in ground camera images \cite{article:cnnsmoke2023};
    \item Computer vision models for burned area segmentation in satellite imagery \cite{article:burnseg2024};
    \item LLMs for summarizing wildfire risk reports \cite{article:llmrisk2024};
    \item PINN applications for wildfire propagation (Section~\ref{sec:pignn}), with physics-based modeling \cite{article:raissi2019,article:karniadakis2021} and specialized surveys \cite{article:cuomo2022,article:lu2021deepxde};
    \item Koopman operator for nonlinear dynamics of environmental systems, grounded in fluid mechanics \cite{article:rowley2009,article:mezic2013} with neural extensions \cite{article:lusch2018,article:takeishi2017};
    \item LLM-based autonomous agents \cite{article:xi2023,article:wang2024survey} with external tool use \cite{article:schick2023,article:patil2023} and reflective memory \cite{article:shinn2023}, enabling task orchestration \cite{article:park2023}.
\end{itemize}
...
\textbf{Nauber Gois}: Conceptualization, Methodology, Software, Writing --- Original Draft, Writing --- Review \& Editing.\\
\textbf{Alexandre Lobo}: Conceptualization, Methodology, Supervision, Writing --- Review \& Editing.\\
