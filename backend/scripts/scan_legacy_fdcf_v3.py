#!/usr/bin/env python3
"""Quick check legacy FDCF fire counts per file - fixed for numpy 2.0."""
...
for fname in sorted(os.listdir(BASE)):
    ...
    if np.ndim(mask_var) == 0:
        ds.close()
        continue
    
    mask = np.asarray(mask_var, dtype=int)
    if isinstance(mask, np.ma.MaskedArray):
        mask = mask.data
    
    fire = np.zeros_like(mask, dtype=bool)
    for v in [200, 201, 215, 220]:
        fire = fire | (mask == v)
    ...
    ce = 0
    for i in range(min(n_fire, 5000)):
        yi, xi = fi[0][i], fi[1][i]
