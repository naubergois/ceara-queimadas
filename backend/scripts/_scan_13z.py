#!/usr/bin/env python3
"""Analyze GOES-19 13:50Z scan for fire detection in Ceará."""
...
c07_path = os.path.join(BASE, 'OR_ABI-L2-CMIPF-M6C07_G19_s20261571350216_e20261571359536_c20261571359589.nc')
c13_path = os.path.join(BASE, 'OR_ABI-L2-CMIPF-M6C13_G19_s20261571350216_e20261571359536_c20261571359573.nc')
...
results = {'satellite': 'GOES-19', 'timestamp': extract_ts(c07_path), 'pixels_ceara': pixels_ceara, 'valid_pixels': valid}
...
if valid > 0:
    ...
    # Compare with previous scan (12:30Z)
    prev_path = os.path.join(BASE, 'goes19_doy157_analysis.json')
    if os.path.exists(prev_path):
        with open(prev_path) as f:
            prev = json.load(f)
        print(f'\n--- Comparison: 12:30Z vs 13:50Z ---')
        print(f'12:30Z: T07={prev.get("t07_range",{}).get("mean","N/A")}K | fires=0')
        print(f'13:50Z: T07={t07_mean}K | fires={len(fire_315)}')
        # Detect if cloud cleared
        if prev.get('t07_range',{}).get('mean', 0) < 210 and t07_mean > 290:
            print('CLOUD CLEARANCE DETECTED: T07 mean increased from ~201K to ~{:.0f}K'.format(t07_mean))

ds7.close()
ds13.close()

# Save results
out_path = os.path.join(BASE, 'goes19_doy157_13z_analysis.json')
